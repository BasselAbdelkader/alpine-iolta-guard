...
class AdvancedThreatDetectionMiddleware(MiddlewareMixin):
    ...
    def detect_behavioral_anomalies(self, request, client_ip):
        ...
        param_set = cache.get(hour_key, set())
        ...
        cache.set(hour_key, param_set, 3600)
    ...
    def detect_session_anomalies(self, request):
        ...
        if hasattr(request, 'session') and request.user.is_authenticated:
            ...
            user_sessions = cache.get(user_sessions_key, set())
            current_session = request.session.session_key
            
            if current_session:
                user_sessions.add(current_session)
                cache.set(user_sessions_key, user_sessions, 3600)
    ...
    def add_to_watchlist(self, ip):
        ...
        watchlist = cache.get(watchlist_key, set())
        watchlist.add(ip)
        cache.set(watchlist_key, watchlist, 86400)  # 24 hours
