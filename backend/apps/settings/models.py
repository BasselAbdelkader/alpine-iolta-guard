...
class CheckSequence(models.Model):
    ...
    next_reference_number = models.IntegerField(
        default=1001,
    ...
    def __str__(self):
        return f"{self.bank_account.account_name} - Next: {self.next_reference_number}"
    ...
    def get_next_numbers(cls, bank_account, count):
        ...
        IMPORTANT: Always syncs with BankAccount.next_reference_number as the authoritative source.
        """
        with transaction.atomic():
            # Get or create sequence for this bank account
            sequence, created = cls.objects.select_for_update().get_or_create(
                bank_account=bank_account,
                defaults={'next_reference_number': bank_account.next_reference_number or 1001}
            )

            # BUGFIX: Sync with BankAccount.next_reference_number (authoritative source)
            # If user edited the Next Check # via UI, BankAccount.next_reference_number will be updated
            # We need to use that value, not the CheckSequence value
            bank_account.refresh_from_db()
            if bank_account.next_reference_number and bank_account.next_reference_number > sequence.next_reference_number:
                # User manually updated the check number via UI - use their value
                start_number = bank_account.next_reference_number
            else:
                # Use the sequence value
                start_number = sequence.next_reference_number
            ...
            sequence.next_reference_number = reference_numbers[-1] + 1
            sequence.last_assigned_date = timezone.now()
            sequence.save()

            # Update BankAccount.next_reference_number to keep them in sync
            bank_account.next_reference_number = reference_numbers[-1] + 1
            bank_account.save(update_fields=['next_reference_number', 'updated_at'])
