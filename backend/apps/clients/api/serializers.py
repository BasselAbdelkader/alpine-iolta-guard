...
class ClientSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Client
        fields = [
            'id', 'client_number', 'first_name', 'last_name', 'full_name',
...
class ClientListSerializer(serializers.ModelSerializer):
    ...
    def get_has_cases(self, obj):
        return obj.cases.exists()

    def get_cases(self, obj):
        """Return all cases for this client"""
        cases = obj.cases.all().order_by('-created_at')
        result = []
        for case in cases:
            result.append({
                'id': case.id,
                ...
                'formatted_balance': case.get_formatted_balance(),
