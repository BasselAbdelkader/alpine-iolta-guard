...
def smart_delete_client(request, client_id):
    """AJAX endpoint to smart delete a client: permanent delete if no records, otherwise mark inactive"""
    try:
        ...
        if has_transactions or has_cases or current_balance != 0:
            # Client has records - mark as inactive instead of deleting
            ...
            return JsonResponse({
                'success': True,
                'action': 'deactivated',
                'message': f'Client "{client_name}" has been marked as inactive due to existing records (cases/transactions).',
                'client_id': client_id
            })
        else:
...
class CaseIndexView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['clients'] = Client.objects.filter(is_active=True).order_by('first_name', 'last_name')
...
class CaseDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for transaction in all_transactions:
            ...
            # Determine client
            client_name = f"{transaction.client.first_name} {transaction.client.last_name}" if transaction.client else "-"
...
def print_clients_report(request):
    ...
    for client in Client.objects.filter(is_active=True).order_by('first_name', 'last_name'):
        balance = client.get_current_balance()
