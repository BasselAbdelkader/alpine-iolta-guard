...
class ClientForm(SecureModelForm):
    class Meta:
        model = Client
        fields = [
            'first_name', 'last_name', 'email', 'phone', 'address', 'city', 'state', 'zip_code', 'is_active'
        ]
        widgets = {
    ...
    def clean(self):
        """Validate that the combination of first_name and last_name is unique"""
        ...
        if first_name:
            ...
            if existing_clients.exists():
                ...
                raise ValidationError({
                    'first_name': f'A client named "{display_name}" already exists in the system. '
