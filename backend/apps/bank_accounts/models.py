...
class BankAccount(models.Model):
    ...
    def get_current_balance(self):
        ...
        COMPLIANCE CONTROL #3: Excludes transactions with pending approval.
        """
        # Get all non-voided transactions for this bank account in chronological order
        # COMPLIANCE CONTROL #3: Exclude transactions with pending approval
        transactions = self.bank_transactions.exclude(
            status='voided'
        ).exclude(
            approval__status='pending'  # CONTROL #3: Exclude pending approvals
...
class BankTransaction(models.Model):
    ...
    IMPORTANT: transaction_type values MUST be uppercase:
    - 'DEPOSIT' (not 'Deposit' or 'deposit')
    - 'WITHDRAWAL' (not 'Withdrawal' or 'withdrawal')
    - 'TRANSFER_OUT' (not 'Transfer_Out' or 'transfer_out')

    The CSV importer enforces uppercase values during import.
    All queries and balance calculations expect uppercase values.
    If you find mixed case values, run: backend/fix_transaction_types.py
    """
    ...
    def clean(self):
        """
        Model validation - enforce uppercase transaction_type
        """
        from django.core.exceptions import ValidationError

        # Enforce uppercase transaction_type
        if self.transaction_type and self.transaction_type != self.transaction_type.upper():
            raise ValidationError({
                'transaction_type': f'Transaction type must be uppercase. Use "{self.transaction_type.upper()}" instead of "{self.transaction_type}".'
            })

    def save(self, *args, **kwargs):
        # === ENFORCE UPPERCASE TRANSACTION_TYPE ===
        # Defensive measure: Auto-convert to uppercase if not already
        # This ensures compliance with balance calculation queries
        if self.transaction_type:
            self.transaction_type = self.transaction_type.upper()

        # === AUTO-LINK VENDOR_ID FROM PAYEE ===
        # If a payee is provided but no vendor is linked, try to find matching vendor
        if self.payee and not self.vendor_id:
            from apps.vendors.models import Vendor
            try:
                # Try exact match first
                vendor = Vendor.objects.filter(vendor_name__iexact=self.payee).first()
                if vendor:
                    self.vendor = vendor
            except Exception:
                pass  # If vendor lookup fails, continue without vendor_id
        ...
        if is_new and self.transaction_type in ['WITHDRAWAL', 'TRANSFER_OUT']:
            logger.info(f"DEBUG: is_new={is_new}, type={self.transaction_type}, status={self.status}, client={self.client}, case={self.case}, amount={self.amount}")
        ...
        if (not self.pk and  # New transaction only
            ...
            not self.reference_number and
            self.status != 'voided'):
            self.reference_number = self.bank_account.get_next_check_number()
        ...
        # Note: reference_number is now the single field for check numbers
        # No synchronization needed as deprecated check_number field was removed

        # ===================================================================
        # COMPLIANCE CONTROL #1: NEGATIVE BALANCE PREVENTION (Model Layer)
        # ===================================================================
        # CRITICAL: This must run BEFORE save() to prevent negative balances
        # Secondary validation at model level (belt and suspenders approach)
        # This catches any transactions created directly via model (bypassing serializer)
        # ===================================================================
        if is_new and self.transaction_type in ['WITHDRAWAL', 'TRANSFER_OUT'] and self.status != 'voided':
            if self.client and self.case and self.amount:
                from decimal import Decimal
                import logging
                logger = logging.getLogger(__name__)

                # Get current balances BEFORE this withdrawal
                client_balance = self.client.get_current_balance()
                case_balance = self.case.get_current_balance()
                withdrawal_amount = Decimal(str(self.amount))

                logger.info(f"NEGATIVE BALANCE CHECK: Withdrawal={withdrawal_amount}, Client={client_balance}, Case={case_balance}")

                # CRITICAL: Prevent negative balances (illegal commingling)
                if withdrawal_amount > client_balance:
                    logger.warning(f"REJECTED: Withdrawal {withdrawal_amount} > Client balance {client_balance}")
                    raise ValueError(
                        f'TRUST ACCOUNT COMPLIANCE VIOLATION: '
                        f'Insufficient client funds. '
                        f'Client "{self.client.full_name}" has ${client_balance:,.2f} available. '
                        f'Cannot withdraw ${withdrawal_amount:,.2f}. '
                        f'Negative balances violate ABA Model Rule 1.15 (Safekeeping Property).'
                    )

                if withdrawal_amount > case_balance:
                    logger.warning(f"REJECTED: Withdrawal {withdrawal_amount} > Case balance {case_balance}")
                    raise ValueError(
                        f'TRUST ACCOUNT COMPLIANCE VIOLATION: '
                        f'Insufficient case funds. '
                        f'Case "{self.case.case_title}" has ${case_balance:,.2f} available. '
                        f'Cannot withdraw ${withdrawal_amount:,.2f}. '
                        f'Negative balances violate ABA Model Rule 1.15 (Safekeeping Property).'
                    )
        ...
        # ===================================================================
        # CRITICAL: High-value withdrawals require approval before affecting balance
        # Implements maker-checker pattern for fraud prevention
        # ===================================================================
        if is_new and self.transaction_type in ['WITHDRAWAL', 'TRANSFER_OUT'] and self.status != 'voided':
            # Check if approval is required for this transaction
            from apps.settings.models import LawFirm
            from decimal import Decimal

            firm = LawFirm.get_active_firm()
            if firm and firm.approval_threshold:
                threshold = Decimal(str(firm.approval_threshold))
                transaction_amount = Decimal(str(self.amount))

                # If transaction amount >= threshold, it will need approval
                # Note: We don't create the approval here - we do it AFTER save()
                # This is because we need the transaction ID first
                self._requires_approval = transaction_amount >= threshold
            else:
        ...
        # ===================================================================
        # COMPLIANCE CONTROL #3: Create TransactionApproval if Required
        # ===================================================================
        if hasattr(self, '_requires_approval') and self._requires_approval:
            try:
                from django.contrib.auth.models import User

                # Get the user who created this transaction
                # Try to get from audit_user parameter, otherwise use 'system'
                creator_username = audit_user if audit_user else 'system'
                try:
                    creator = User.objects.get(username=creator_username)
                except User.DoesNotExist:
                    # Fall back to first superuser if audit user not found
                    creator = User.objects.filter(is_superuser=True).first()
                    if not creator:
                        # No users available - skip approval creation
                        logger.warning(f"Cannot create approval for transaction {self.pk}: No valid user found")
                        return
                ...
                if not hasattr(self, 'approval'):
                    ...
                    TransactionApproval.objects.create(
                        ...
                        created_by=creator,
                        status='pending',
                        request_notes=f'High-value transaction (${self.amount:,.2f}) requires approval',
                        ip_address=audit_ip
                    )

                    logger.info(f"✅ Created approval request for transaction {self.transaction_number} (${self.amount:,.2f})")
            except Exception as e:
                # Log the error but don't fail the transaction save
                import logging
                logger = logging.getLogger(__name__)
                logger.error(f"Approval creation failed for transaction {self.pk}: {str(e)}")

        # ===================================================================
        # COMPLIANCE CONTROL #10: FRAUD DETECTION & ALERTS
        # ===================================================================
        # CRITICAL: Check transaction for suspicious patterns
        # Implements real-time fraud detection for embezzlement prevention
        # ===================================================================
        if is_new and not kwargs.get('skip_fraud_check', False):
            try:
                from apps.fraud_detection.services import FraudDetectionEngine

                engine = FraudDetectionEngine()
                alerts = engine.check_transaction(self)

                if alerts:
                    logger.warning(
                        f"🚨 FRAUD ALERTS: {len(alerts)} alert(s) created for "
                        f"transaction #{self.transaction_number} (${self.amount})"
                    )
            except Exception as e:
                # Log the error but don't fail the transaction save
                import logging
                logger = logging.getLogger(__name__)
                logger.error(f"Fraud detection failed for transaction {self.pk}: {str(e)}")
    ...
    def void_transaction(self, void_reason, voided_by=None, ip_address=None):
        ...
        if self.status == 'voided':
            raise ValueError("Transaction is already voided")
                ...
                # Check if voiding would cause a negative balance (for DEPOSITs)
...
class TransactionApproval(models.Model):
    """
    Tracks approval workflow for high-value transactions.
    Implements maker-checker pattern (dual authorization).

    Compliance Requirements:
    - ABA Model Rule 1.15: Safeguarding property
    - Two-person approval for transactions ≥ threshold (default $10,000)
    - Creator cannot approve own transaction (maker-checker)
    - Only users with can_approve_transactions permission can approve

    Workflow:
    1. User creates transaction ≥ threshold
    2. System auto-creates TransactionApproval with status='pending'
    3. Transaction does NOT affect balance while pending
    4. Approver reviews and approves/rejects
    5. If approved: transaction affects balance
    6. If rejected: transaction is voided
    """
    ...
    transaction = models.OneToOneField(
        ...
        help_text='Transaction requiring approval'
    )
    ...
    reviewed_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        help_text='User who approved/rejected the transaction'
    )
    ...
    request_notes = models.TextField(
        blank=True,
        help_text='Reason for transaction / why approval is needed'
    )
    ...
    class Meta:
        ...
        # Database-level constraint to prevent self-approval
        constraints = [
            models.CheckConstraint(
                check=(
                    models.Q(reviewed_by__isnull=True) |  # Not yet reviewed
                    ~models.Q(created_by=models.F('reviewed_by'))  # Different users
                ),
                name='prevent_self_approval',
                violation_error_message='Segregation of Duties Violation: Creator cannot approve their own transaction'
            ),
        ]

    def __str__(self):
        return f"Approval for {self.transaction.transaction_number} - {self.status}"

    def approve(self, user, notes='', ip_address=None):
        """
        ...
        Raises:
            ValidationError: If user is creator (maker-checker violation)
        ...
        # Validation #1: Cannot approve if already processed
        if self.status != 'pending':
            raise ValidationError(
                f'Cannot approve - transaction is already {self.get_status_display()}'
            )

        # Validation #2: Maker-checker enforcement (creator cannot approve)
        if self.created_by == user:
            raise ValidationError(
                'COMPLIANCE VIOLATION: You cannot approve your own transaction. '
                'Two-person approval requires a different user to approve. '
                '(Maker-Checker Enforcement)'
            )

        # Validation #3: User must have approval permission
        # Check UserProfile permission, Django permission, or superuser status
        has_permission = False

        if user.is_superuser:
            has_permission = True
        elif user.has_perm('bank_accounts.can_approve_transactions'):
            has_permission = True
        else:
            # Check UserProfile (RBAC)
            try:
                profile = user.profile
                if profile.can_approve_transactions:
                    has_permission = True
            except:
                pass
        ...
        if not has_permission:
            raise ValidationError(
                f'User {user.username} does not have permission to approve transactions. '
                f'Required: Superuser, Django permission (bank_accounts.can_approve_transactions), '
                f'or UserProfile.can_approve_transactions=True'
            )
        ...
        # Note: Transaction now affects balance (balance calculation excludes pending approvals)
        return True

    def reject(self, user, notes='', ip_address=None):
        """
        ...
        Raises:
            ValidationError: If already approved/rejected
            ValidationError: If no rejection reason provided
        """
        from django.core.exceptions import ValidationError
        from django.utils import timezone

        # Validation #1: Cannot reject if already processed
        if self.status != 'pending':
            raise ValidationError(
                f'Cannot reject - transaction is already {self.get_status_display()}'
            )

        # Validation #2: Rejection reason required
        if not notes or not notes.strip():
            raise ValidationError(
                'Rejection reason is required. Please provide explanation for rejection.'
            )
        ...
        self.transaction.status = 'voided'
        self.transaction.void_reason = f'Rejected by {user.username}: {notes}'
        self.transaction.voided_by = user.username
        self.transaction.voided_date = timezone.now()
    ...
    @property
    def is_self_approval_attempt(self):
        """
        Check if this would be a self-approval (SOD violation).

        COMPLIANCE CONTROL #5: Segregation of Duties
        Returns True if reviewed_by is the same as created_by
        """
        return (
            self.reviewed_by is not None and
            self.created_by == self.reviewed_by
        )
