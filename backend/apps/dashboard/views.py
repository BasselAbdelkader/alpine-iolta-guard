...
class DashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ).exclude(status='voided').exclude(reference_number='').select_related('client', 'vendor')
    ...
    def _get_health_details(self, client_balance_dict, all_clients):
        ...
        uncleared_transactions = BankTransaction.objects.filter(status='pending').select_related('bank_account', 'client', 'vendor').order_by('-transaction_date')
    ...
    def _calculate_trust_health(self, bank_account, bank_register_balance, total_client_balance, client_balance_dict):
        ...
        outstanding_count = BankTransaction.objects.filter(
            ...
            transaction_type='WITHDRAWAL',
            transaction_date__lt=ninety_days_ago
        ).exclude(status='voided').exclude(reference_number='').exclude(status='cleared').count()
