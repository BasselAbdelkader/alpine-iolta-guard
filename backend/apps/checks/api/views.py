...
class CheckViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = BankTransaction.objects.filter(
            ...
            # Only include checks that haven't been assigned a real check number yet
            Q(check_number__isnull=True) | Q(check_number='') | Q(check_number='TO PRINT')
        ).exclude(
            status='voided'
        ).select_related('client', 'case', 'vendor', 'bank_account')

        # Apply filters
        check_number = self.request.query_params.get('check_number')
        payee = self.request.query_params.get('payee')
        from_date = self.request.query_params.get('from_date')
        to_date = self.request.query_params.get('to_date')

        if check_number:
            queryset = queryset.filter(reference_number__icontains=check_number)
        if payee:
    ...
    def print_batch_pdf(self, request):
        ...
        try:
            ...
            print(f"[PDF GENERATE] Found {checks.count()} checks with check_numbers: {list(checks.values_list('check_number', 'id'))}", file=sys.stderr, flush=True)
    ...
    def next_check_number(self, request):
        ...
        sequence, created = CheckSequence.objects.get_or_create(
            ...
            defaults={'next_check_number': 1001}
        )

        print(f"[NEXT CHECK NUMBER] Bank account: {bank_account.account_name}, Next number: {sequence.next_check_number}, Last assigned: {sequence.last_assigned_number}")

        return Response({
            'next_check_number': sequence.next_check_number,
    ...
    def update_next_check_number(self, request):
        ...
        try:
            new_check_number = int(new_check_number)
            if new_check_number < 1:
        ...
        sequence, created = CheckSequence.objects.get_or_create(
            bank_account=bank_account,
            defaults={'next_check_number': new_check_number}
        )

        # Update the next check number
        sequence.next_check_number = new_check_number
        sequence.save()

        print(f"[UPDATE CHECK NUMBER] Updated next check number to {new_check_number} for account {bank_account.account_name}")

        return Response({
            'success': True,
            'next_check_number': sequence.next_check_number,
            'bank_account': bank_account.account_name,
            'message': f'Next check number updated to {new_check_number}'
        })

    @action(detail=False, methods=['POST'], url_path='assign-check-numbers')
    def assign_check_numbers(self, request):
        """
        ...
        try:
            with db_transaction.atomic():
                check_numbers = CheckSequence.get_next_numbers(bank_account, len(check_ids))

                # Assign check numbers to transactions
                results = []
                for txn, check_num in zip(transactions, check_numbers):
                    print(f"[CHECK ASSIGN] Assigning check #{check_num} to transaction ID {txn.id}", file=sys.stderr, flush=True)
                    # BUGFIX: Update both check_number AND reference_number (they should be the same)
                    # Reference was "TO PRINT", now it becomes the actual check number
                    txn.check_number = str(check_num)
                    txn.reference_number = str(check_num)
                    txn.save()
                    results.append({
                        'transaction_id': txn.id,
                        'check_number': str(check_num),
                        'payee': txn.payee or (txn.vendor.vendor_name if txn.vendor else None),
                        'amount': str(txn.amount)
                    })

                print(f"[CHECK ASSIGN] Successfully assigned check numbers: {[r['check_number'] for r in results]}", file=sys.stderr, flush=True)

                return Response({
                    'success': True,
                    'message': f'Assigned check numbers {check_numbers[0]} to {check_numbers[-1]}',
                    'checks': results,
                    'next_check_number': check_numbers[-1] + 1
                })
