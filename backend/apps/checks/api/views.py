...
class CheckViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def next_reference_number(self, request):
        ...
        sequence, created = CheckSequence.objects.get_or_create(
            ...
            defaults={'next_reference_number': 1001}
        )

        print(f"[NEXT CHECK NUMBER] Bank account: {bank_account.account_name}, Next number: {sequence.next_reference_number}, Last assigned: {sequence.last_assigned_number}")

        return Response({
            'next_reference_number': sequence.next_reference_number,
            'bank_account': bank_account.account_name
        })

    @action(detail=False, methods=['POST'], url_path='update-next-check-number')
    def update_next_reference_number(self, request):
        """Update the next check number"""
        from apps.settings.models import CheckSequence
        from apps.bank_accounts.models import BankAccount

        new_reference_number = request.data.get('next_reference_number')
        if not new_reference_number:
            return Response({'error': 'next_reference_number is required'}, status=400)
        ...
        sequence, created = CheckSequence.objects.get_or_create(
            bank_account=bank_account,
            defaults={'next_reference_number': new_reference_number}
        )

        # Update the next check number
        sequence.next_reference_number = new_reference_number
        sequence.save()

        print(f"[UPDATE CHECK NUMBER] Updated next check number to {new_reference_number} for account {bank_account.account_name}")
        ...
        try:
            with db_transaction.atomic():
                ...
                return Response({
                    ...
                    'checks': results,
                    'next_reference_number': reference_numbers[-1] + 1
                })
