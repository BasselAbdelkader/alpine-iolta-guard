...
class CheckPrintQueueView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Include: reference="TO PRINT", check_number="TO PRINT", or has an actual check number
        ...
        ready_to_print = BankTransaction.objects.filter(
            ...
            Q(check_number='TO PRINT') |      # Check number is "TO PRINT"
            Q(check_number__isnull=True) |    # No check number yet
            Q(check_number='') |              # Empty check number
            (Q(check_number__isnull=False) & ~Q(check_number=''))  # Has actual check number
        ).exclude(status='voided').select_related(
            'client', 'vendor', 'case', 'bank_account'
        )

        # Apply search filters
        check_number = self.request.GET.get('check_number', '').strip()
        ...
        if check_number:
            ready_to_print = ready_to_print.filter(check_number__icontains=check_number)
        ...
        ready_to_print = ready_to_print.order_by('transaction_date', 'check_number')
...
class PrintCheckView(LoginRequiredMixin, View):
    ...
    def get(self, request, transaction_number):
        ...
        if not check.check_number or check.check_number == 'TO PRINT':
            check.check_number = check.bank_account.get_next_check_number()
            check.save(update_fields=['check_number'])
        ...
        response['Content-Disposition'] = f'inline; filename="check_{check.check_number}.pdf"'
