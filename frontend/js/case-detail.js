...
async function loadTransactions() {
    ...
    try {
        ...
        const transactionsWithBalance = sortedTransactions.map(txn => {
            ...
            const isVoided = txn.status === 'voided' || (txn.amount === '0.00' && txn.status === false);
                ...
                <table class="table table-striped table-hover align-middle">
                    <thead class="table-dark">
                        <tr>
                            ...
                            <th style="width: 100px;" class="text-center">Check #</th>
...
async function editTransaction(transactionId) {
    try {
        ...
        if (transactionStatus === 'cleared' || transactionStatus === 'reconciled') {
...
async function reissueTransaction(transactionId) {
    ...
    try {
        ...
        if (transaction.status === 'cleared' || transaction.status === 'reconciled') {
            ...
            showErrorNotification(`Cannot reissue ${transaction.status === 'cleared' ? 'Cleared' : 'Reconciled'} transactions.`);
            return;
        }
...
async function submitTransaction() {
    ...
    const transactionData = {
        ...
        reference_number: document.getElementById('transaction_ref').value,
