...
async function voidTransaction(transactionId) {
    ...
    try {
        ...
        if (transaction.status === 'cleared' || transaction.status === 'reconciled') {
            ...
            showCenteredError(`Cannot void ${transaction.status === 'cleared' ? 'cleared' : 'reconciled'} transactions.`);
            return;
        }
...
async function reissueTransaction(transactionId) {
    ...
    try {
        ...
        if (transaction.status === 'cleared' || transaction.status === 'reconciled') {
            // Show centered red error toast
            showCenteredError(`Cannot reissue ${transaction.status === 'cleared' ? 'cleared' : 'reconciled'} transactions.`);
            return;
        }
