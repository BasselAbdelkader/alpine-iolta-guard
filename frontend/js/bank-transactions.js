                ...
                <div class="col-md-2" style="max-width: 180px;">
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" name="amount_display" class="form-control" id="id_amount_display" placeholder="1,000" required>
                ...
                <div class="mb-3">
                    ...
                    <table class="table table-sm table-bordered">
                        ...
                        <tr>
                            ...
                            <td><span class="badge ${txn.status === 'voided' ? 'bg-danger' : txn.status === 'cleared' ? 'bg-success' : 'bg-warning text-dark'}">${txn.status.toUpperCase()}</span></td>
                        </tr>
                        <tr>
...
async function reissueTransaction(transactionId) {
    ...
    try {
        ...
        if (transaction.status === 'cleared' || transaction.status === 'reconciled') {
            ...
            showErrorMessage(`Cannot reissue ${transaction.status === 'cleared' ? 'Cleared' : 'Reconciled'} transactions.`);
            return;
        }
...
function updateAvailableFunds() {
    ...
    if (selectedCaseId && caseSelect) {
        ...
        const displayBalance = rawBalance.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
...
function initializeTransactionForm() {
    ...
    if (clientInput && clientSuggestions) {
        ...
        clientInput.addEventListener('input', function() {
            ...
            // Clear hidden field if user is typing
    ...
    if (payeeInput && suggestions) {
        ...
        payeeInput.addEventListener('input', function() {
            ...
            if (exactMatch) {
                // Auto-fill with proper capitalization
...
function attachCaseDropdownHandler() {
    ...
    console.log('[CASE DROPDOWN] clientSelect:', clientSelect);
    console.log('[CASE DROPDOWN] caseSelect:', caseSelect);
    ...
    if (clientSelect && caseSelect) {
        ...
        clientSelect.addEventListener('change', function() {
            ...
            console.log('[CASE DROPDOWN] Client changed, ID:', clientId);
            ...
            if (clientId) {
                ...
                console.log('[CASE DROPDOWN] Fetching cases for client:', clientId);

                // Load cases for this client using the API
                fetch(`/api/v1/clients/${clientId}/cases/`, {
                    credentials: 'include',
                    headers: {
                ...
                .then(response => {
                    console.log('[CASE DROPDOWN] Response status:', response.status);
                    return response.json();
                })
                .then(data => {
                    console.log('[CASE DROPDOWN] API response:', data);
                    // API returns object with cases array
                    const cases = data.cases || [];
                    console.log('[CASE DROPDOWN] Cases found:', cases.length);
                    if (cases && cases.length > 0) {
                        cases.forEach(caseObj => {
                            const option = document.createElement('option');
                            option.value = caseObj.id;  // Backend uses ID for API calls
                            option.textContent = `${caseObj.case_title || caseObj.case_description}`;  // Display only case title
                            // Store the balance as a data attribute so we can use it when case is selected
                            option.setAttribute('data-balance', caseObj.current_balance || 0);
                            option.setAttribute('data-formatted-balance', caseObj.formatted_balance || '0.00');
                            caseSelect.appendChild(option);
                        ...
                        console.log('[CASE DROPDOWN] No cases found for client');
                    }
                })
                .catch(error => {
                    console.error('[CASE DROPDOWN] Error loading cases:', error);
                });
            }

            // Don't call updateAvailableFunds() here - no case selected yet
            // The funds will be updated when user selects a case
        });
        ...
        console.error('[CASE DROPDOWN] Could not find client or case select elements!');
    }
}
