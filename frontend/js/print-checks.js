    ...
    tbody.innerHTML = checks.map(check => {
        ...
        return `
            <tr>
                ...
                <td><strong>${check.check_number || check.reference_number}</strong></td>
                <td>${date}</td>
...
async function printCheck(checkId) {
    ...
    if (check.check_number && check.check_number !== 'TO PRINT') {
        // REQUIREMENT: Show preview before printing
        const confirmed = await showCheckPreview(check, check.check_number);
        if (confirmed) {
...
async function loadNextCheckNumber() {
    try {
        ...
        // Get all bank accounts (remove invalid filters that cause 400 error)
        const accountsResponse = await api.get('/v1/bank-accounts/accounts/');
        console.log('Bank accounts response:', accountsResponse);
        console.log('Response type:', typeof accountsResponse);
        console.log('Response keys:', Object.keys(accountsResponse));
        console.log('Response.results:', accountsResponse.results);
        ...
        if (Array.isArray(accountsResponse)) {
            ...
            console.error('Unexpected response format:', accountsResponse);
        }
        ...
        if (accounts.length > 0) {
            ...
            const nextCheckNumber = accounts[0].next_check_number || 1001;
            console.log('Next check number from account:', nextCheckNumber);
            ...
            // If no accounts found, try to get any bank account
            const allAccountsResponse = await api.get('/v1/bank-accounts/accounts/');
            console.log('All accounts response:', allAccountsResponse);

            let allAccounts = [];
            if (Array.isArray(allAccountsResponse)) {
                allAccounts = allAccountsResponse;
            } else if (allAccountsResponse.results && Array.isArray(allAccountsResponse.results)) {
                allAccounts = allAccountsResponse.results;
            } else if (allAccountsResponse.data && Array.isArray(allAccountsResponse.data)) {
                allAccounts = allAccountsResponse.data;
            }

            if (allAccounts.length > 0) {
                currentBankAccountId = allAccounts[0].id;
                const nextCheckNumber = allAccounts[0].next_check_number || 1001;
                console.log('Using first available account. Next check number:', nextCheckNumber);
                document.getElementById('nextCheckNumberDisplay').value = nextCheckNumber;
            } else {
                console.error('No bank accounts found at all!');
